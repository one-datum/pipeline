...
rule noise_infer:
    ...
    output:
        get_results_filename("{dataset}/noise/infer-{n}.fits")
    params:
        config=config["noise_config_file"],
    conda:
        "../envs/environment.yml"
    log:
        get_log_filename("{dataset}/noise/infer-{n}.log")
    shell:
        """
...
rule noise_combine:
    ...
    output:
        get_results_filename("{dataset}/noise/combine.fits")
    params:
        config=config["noise_config_file"],
    conda:
        "../envs/environment.yml"
    log:
        get_log_filename("{dataset}/noise/combine.log")
    shell:
        """
...
rule noise_postprocess:
    ...
    conda:
        "../envs/environment.yml"
    log:
        get_log_filename("{dataset}/noise/process.log")
    shell:
        """
...
rule noise_apply:
    input:
        noise_model=get_results_filename("{dataset}/noise/process.fits"),
